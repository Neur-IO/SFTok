...
from modeling.modules import EMAModel, ReconstructionLoss_Stage1, ReconstructionLoss_Stage2, ReconstructionLoss_Single_Stage, MLMLoss, ARLoss, ReconstructionLoss_MLMStage1
...
from demo_util import get_titok_tokenizer, get_tatitok_tokenizer, sample_fn, get_sftok_tokenizer
...
def create_model_and_loss_module(config, logger, accelerator,
    ...
    elif model_type == "sftok":
        model_cls = SFTok
        loss_cls = ReconstructionLoss_Stage2 if config.model.vq_model.finetune_decoder else ReconstructionLoss_MLMStage1
    elif model_type == "tatitok":
        model_cls = TATiTok
        loss_cls = ReconstructionLoss_Single_Stage
    elif model_type == "maskgit":
        if config.model.img_generator.model_type == "ViT":
            model_cls = ImageBert
        elif config.model.img_generator.model_type == "UViT":
            model_cls == UViTBert
        else:
    ...
    if accelerator.is_main_process:
        ...
        elif model_type in ["sftok"]:
            input_data = [
                torch.randn(1, 3, config.dataset.preprocessing.crop_size, config.dataset.preprocessing.crop_size),
                torch.ones(1, 256, dtype=torch.long)
            ]
            model_summary_str = summary(model, input_data=input_data, depth=5,
            col_names=("input_size", "output_size", "num_params", "params_percent", "kernel_size", "mult_adds"))
            logger.info(model_summary_str)
            pass
...
def train_one_epoch(config, logger, accelerator,
    ...
    for i, batch in enumerate(train_dataloader):
        ...
        if pretrained_tokenizer is not None:
            ...
            sampled_ids = proxy_codes
        else:
            proxy_codes = torch.randint(0, 1024, (images.size(0), 256), device=images.device, dtype=torch.long)

        with accelerator.accumulate([model, loss_module]):
            if model_type == "titok":
                reconstructed_images, extra_results_dict = model(x=images, use_semantic=False)
                ...
                if config.training.guided_mask:
                    with torch.no_grad():
                        guided_mask = accelerator.unwrap_model(model).generate_guided_mask(x=images, proxy_codes=proxy_codes, use_semantic=False, mask_all=True)
                else:
                    guided_mask = None

                reconstructed_images, extra_results_dict, masks, _ = model(x=images, 
                    proxy_codes=proxy_codes, use_semantic=False, 
                    mask_all=config.training.single_step_generation,
                ...
                if config.model.vq_model.finetune_decoder:
                    ...
                    autoencoder_loss, loss_dict = loss_module(
                        ...
                        extra_results_dict,
                        masks=masks,
                        use_mlm=config.training.use_mlmloss,
                    )    
            elif model_type == "tatitok":
        ...
        if accelerator.sync_gradients:
            ...
            if (global_step + 1) % config.experiment.generate_every == 0 and accelerator.is_main_process:
                ...
                reconstruct_images(
                    ...
                    proxy_codes=proxy_codes[:config.training.num_generated_images],
                    logger=logger,
                    config=config,
        ...
        with accelerator.accumulate([model]):
            ...
            if model_type == "maskgit":
                logits = model(masked_tokens, conditions,
                            cond_drop_prob=config.model.img_generator.class_label_dropout)
                loss, loss_dict= loss_module(logits, input_tokens, weights=masks)
            elif model_type == "rar":
                condition = unwrap_model.preprocess_condition(
                    conditions, cond_drop_prob=config.model.img_generator.class_label_dropout
                )
...
def eval_reconstruction(
    ...
    for batch in eval_loader:
        ...
        if model_type == "titok":
            ...
            if config.training.single_step_generation:
                if pretrained_tokenizer is not None:
                    pretrained_tokenizer.eval()
                    proxy_codes = pretrained_tokenizer.encode(images)
                else:
                    proxy_codes = None
                enc_tokens, model_dict = local_model.encode(images)
                reconstructed_images, _, _ = local_model.decode(z_quantized=enc_tokens, 
                                                            proxy_codes=proxy_codes,
                                                            drop_prob=0.0,
                                                            mask_all=True)
            else:
                enc_tokens, model_dict = local_model.encode(images)
                reconstructed_images = local_model.generate_codes(condition=enc_tokens, config=config)
        elif model_type == "tatitok":
        ...
        if pretrained_tokenizer is not None:
            if model_type == "sftok":
                if config.training.single_step_generation:
                    reconstructed_images = reconstructed_images.argmax(2).view(reconstructed_images.shape[0], 16, 16)
                else:
                    reconstructed_images = reconstructed_images.view(reconstructed_images.shape[0], 16, 16)
                reconstructed_images = pretrained_tokenizer.decode(reconstructed_images)
            else:
...
def reconstruct_images(model, original_images, fnames, accelerator, 
    ...
    if model_type == "titok":
        ...
        if config.training.single_step_generation:
            reconstructed_images, _, _ = accelerator.unwrap_model(model).decode(z_quantized=enc_tokens, 
                                                                        proxy_codes=proxy_codes,
                                                                        drop_prob=0.0,
                                                                        mask_all=True)
        else:
            reconstructed_images = accelerator.unwrap_model(model).generate_codes(condition=enc_tokens, config=config)
    elif model_type == "tatitok":
...
def generate_images(model, tokenizer, accelerator, 
    ...
    generated_image = sample_fn(
        ...
        tokenizer,
        guidance_scale=config.model.img_generator.get("guidance_scale", 3.0),
        guidance_decay=config.model.img_generator.get("guidance_decay", "constant"),
        guidance_scale_pow=config.model.img_generator.get("guidance_scale_pow", 3.0),
        randomize_temperature=config.model.img_generator.get("randomize_temperature", 2.0),
        softmax_temperature_annealing=config.model.img_generator.get("softmax_temperature_annealing", False),
        num_sample_steps=config.model.img_generator.get("num_steps", 8),
        device=accelerator.device,
        return_tensor=True,
        config=config
    )
