...
class ReconstructionLoss_MLMStage1(torch.nn.Module):
    def __init__(
        self,
        config
    ):
    ...
    def forward(self,
                ...
                use_mlm: bool = True,
                soft_proxycodes: bool = False,
                ) -> Tuple[torch.Tensor, Mapping[Text, torch.Tensor]]:
        return self._forward_generator(target_codes, reconstructions, quantizer_loss, masks, use_mlm, soft_proxycodes)
    ...
    def _forward_generator(self,
        ...
        if use_mlm:
            if soft_proxycodes:
                ...
                log_probs = F.log_softmax(reconstructions, dim=-1)  # (bs, 256, 1024)

                # 软标签交叉熵：-∑(target_prob * log(pred_prob))
                reconstruction_loss = -torch.sum(target_codes * log_probs, dim=-1)  # (bs, 256)

                if masks is not None:
                    weights = masks.to(reconstruction_loss)
                    loss_weights = (1.0 - weights) * self.loss_weight_unmasked_token + weights
                    reconstruction_loss = (reconstruction_loss * loss_weights).sum() / (loss_weights.sum() + 1e-8)
                else:
                ...
                loss_fct = torch.nn.CrossEntropyLoss(label_smoothing=self.label_smoothing,
                                                           reduction="none")
                reconstructions = rearrange(reconstructions, "b n c -> b c n")
                reconstruction_loss = loss_fct(reconstructions, target_codes)
                if masks is not None:
                    weights = masks.to(reconstruction_loss)
                    loss_weights = (1.0 - weights) * self.loss_weight_unmasked_token + weights
                    reconstruction_loss = (reconstruction_loss * loss_weights).sum() / (loss_weights.sum() + 1e-8)
